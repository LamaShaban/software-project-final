...
$product = $database->query("SELECT * FROM products  WHERE product_num = '8'");
...
if(isset($_POST['addfav'])){
  if(isset($_SESSION['user_id'])){
    ...
    $check_wishlist_numbers = $database->prepare("SELECT * FROM wishlist WHERE product_num = '8' AND user_id = '$user_id' ");
      ...
      foreach ($rows as $row)  {
        ...
        $adddata = $database->prepare("INSERT INTO wishlist (user_id,product_num,product_name,product_img,price) VALUES( '$user_id' ,'8','$product_name','$product_img','$price')");
        $adddata->execute();
...
<head>
    ...
    <title>0.125watt Resistor</title>
