...
if(isset($_POST['delete'])){
  ...
  $delete_cart_item = $database->prepare("DELETE FROM `wishlist` WHERE product_num = ?");
...
$fav_products = $database->query("SELECT * FROM wishlist WHERE user_id = '$user_id'");
$rows = $fav_products->fetchAll(PDO::FETCH_ASSOC);
?>
<!DOCTYPE html>
<html>
<head>
		...
		<table>
			...
			<th>Product picture</th>
				...
				<tr>
					...
					<td><img src="<?php echo $row['product_img']; ?>" alt="0.5R" class="product_img"></td>
            ...
            <input type="hidden" name="product_num" value="<?= $row['product_num']; ?>">

            <input type="submit" value="delete" onclick="return confirm('delete this from cart?');" class="delete-btn add-wishlist-btn" name="delete"></form></td>
					<?php
						}
          ?>
			</table>
