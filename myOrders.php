...
<body>
    ...
    <?php
      if($user_id == ''){
         ...
         $select_orders = $database->prepare("SELECT * FROM `orders` WHERE user_id = ?");
         $select_orders->execute([$user_id]);
         if($select_orders->rowCount() > 0){
            while($fetch_orders = $select_orders->fetch(PDO::FETCH_ASSOC)){
   ?>
   <div class="box bg-light p-5 rounded" style="margin-top:90px;width:700px;margin-left:90px;">
      ...
      <p> payment status : <span style="color:<?php if($fetch_orders['status'] == 'pending'){ echo 'red'; }else{ echo 'green'; }; ?>"><?= $fetch_orders['status']; ?></span> </p>
   </div>
   <?php
      }
      }else{
