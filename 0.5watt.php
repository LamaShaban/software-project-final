...
$product = $database->query("SELECT * FROM products  WHERE product_num = '12'");
...
if(isset($_POST['addfav'])){
  if(isset($_SESSION['user_id'])){
    ...
    $check_wishlist_numbers = $database->prepare("SELECT * FROM wishlist WHERE product_num = '12' AND user_id = '$user_id' ");
      ...
      foreach ($rows as $row)  {
        ...
        $adddata = $database->prepare("INSERT INTO wishlist (user_id,product_num,product_name,product_img,price) VALUES( '$user_id' ,'12','$product_name','$product_img','$price')");
        $adddata->execute();
