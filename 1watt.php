...
$product = $database->query("SELECT * FROM products  WHERE product_num = '2'");
$rows = $product->fetchAll(PDO::FETCH_ASSOC);

if(isset($_POST['addfav'])){
  
    $check_wishlist_numbers = $database->prepare("SELECT * FROM wishlist WHERE product_num = '2' AND user_id = '$user_id' ");
      ...
      foreach ($rows as $row)  {
        ...
        $adddata = $database->prepare("INSERT INTO wishlist (user_id,product_num,product_name,product_img,price) VALUES( '$user_id' ,'2','$product_name','$product_img','$price')");
        $adddata->execute();
